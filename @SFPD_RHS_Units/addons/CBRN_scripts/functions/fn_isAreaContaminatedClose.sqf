/*
	...
	EXAMPLE
		[getPos player, 50] call CBRN_fnc_isAreaContaminatedClose
*/
...
private _returnArray = [false, false, false];
{
	...
	_returnArray set [_forEachIndex, -1 < (_x findIf {
		...
		if (!isNil "_x") then {
			...
			if ((10 + _dist) >= abs (_pos vectorDistance _x)) exitWith {true};
		};
	})];
} forEach CBRN_contaminatedAreas;

{_returnArray set [_forEachIndex, (-1 < (_x findIf {((_x # 1) + _dist) >= abs (_pos vectorDistance (_x # 0))})) or (_returnArray # _forEachIndex)];} forEach CBRN_permanentContaminatedAreas;
