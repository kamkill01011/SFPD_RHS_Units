...
["CBRN_AI_attemptTreatment", {
	...
	//Now check if we have adenosine
	if !("ACE_adenosine" in items _unit) exitWith {};
	...
	["CBRN_AI_kneel", [_unit, 3 + random 3, {
		...
		//Remove our item
		_this removeItem "ACE_adenosine";
		
		//Call our event
		["ace_treatmentSucceded", [_this, _this, "hand_l", "Adenosine"]] call CBA_fnc_localEvent;
		
		//Spawn that nice litter
		[_this, _this, "hand_l", "Adenosine"] call ace_medical_fnc_createLitter;
