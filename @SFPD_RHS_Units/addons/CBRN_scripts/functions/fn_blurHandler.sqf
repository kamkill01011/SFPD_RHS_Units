...
["CBRN_unitExposureChanged", {
	...
	_blurAmount = 0;
	if (_exposure < CBRN_exposureLimpThreshold) then {
		_blurAmount = (_exposure / CBRN_exposureLimpThreshold) * 0.2;
	} else {
		_blurAmount = (((_exposure-CBRN_exposureLimpThreshold) / (CBRN_exposureKnockoutThreshold-CBRN_exposureLimpThreshold)) * 0.4) + 0.2;
	};
	if ((_unit getVariable ["CBRN_isExposedToCS", false])) then {
		_blurAmount = 6;
	};
