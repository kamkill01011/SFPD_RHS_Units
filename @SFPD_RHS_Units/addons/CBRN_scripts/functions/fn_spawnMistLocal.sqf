/*
	...
	PARAMETERS:
		...
		EndTime
			When the mist should disappear (time + _lifetime)
	...
	EXAMPLE
		[getPosASL player, 25, time + 60, 1] call CBRN_fnc_spawnMistLocal
*/

params ["_position", "_radius", "_endTime", "_contaminationType",
	["_verticleSpread", 2],
	["_thickness", 1]
];

private _lifetime = round(_endTime - time);
...
private _color = [_contaminationColor + [0.001], _contaminationColor + [0.05], [1,1,1,0]];
