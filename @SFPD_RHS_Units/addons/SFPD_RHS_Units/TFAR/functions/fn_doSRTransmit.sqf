...
private _hintText = format[
                            localize (["STR_TFAR_core_transmit", "STR_TFAR_core_additional_transmit"] select _additional),
                            format [
                                    "%1<img size='1.5' image='%2'/>",
                                    ([_radio, "displayName", ""] call TFAR_fnc_getWeaponConfigProperty) select [0, _MAX_RADIONAME_LEN],
...
private _range = [_radio, "tf_range", 0] call TFAR_fnc_getWeaponConfigProperty;
private _pluginCommand = format[
                                "TANGENT	PRESSED	%1%2	%3	%4	%5",
                                _frequency,
                                _radio call TFAR_fnc_getSwRadioCode,
                                (_range) * (call TFAR_fnc_getTransmittingDistanceMultiplicator),
...
[TFAR_currentUnit, _range] call TFAR_fnc_updateRange;
